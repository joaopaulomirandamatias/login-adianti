...
class SystemRegistrationForm extends TPage
{
    ...
    function __construct()
    {
        ...
        $cadastro   = new TLabel( _t('User registration'));
        $cadastro->style = 'font-size:25px;text-align: center;color: #66b864;padding-bottom: 15px; ';
        ...
        $this->form->addAction( _t('Save'),  new TAction([$this, 'onSave']), 'far:save')->{'class'} = 'btn btn-sm btn-success';
        $this->form->addAction( _t('Clear'), new TAction([$this, 'onClear']), 'fa:eraser' )->{'class'} = 'btn btn-sm btn-danger';
        ...
        $email->setSize('100%');
          ...
          $this->form->addFields( [$cadastro] );
        $this->form->addFields( [new TLabel(_t('Login'), 'green')],    [$login] );
        $this->form->addFields( [new TLabel(_t('Name'), 'green')],     [$name] );
        $this->form->addFields( [new TLabel(_t('Email'), 'green')],    [$email] );
        $this->form->addFields( [new TLabel(_t('Password'), 'green')], [$password] );
        $this->form->addFields( [new TLabel(_t('Password confirmation'), 'green')], [$repassword] );
